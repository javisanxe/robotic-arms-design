...
vertices=8;   %Numero de vertices (PAR)
Tfigura=40;    %Duración en realizar la trayectoria de figura geométrica
...
V(:,:,i)=transl([R2*cos(Angulo*(i-1)) R2*sin(Angulo*(i-1)) 0]);
else            %Si es impar
V(:,:,i)=transl([R *cos(Angulo*(i+2)) R *sin(Angulo*(i+2)) 0]);
end
